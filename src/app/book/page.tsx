...
function BookingForm() {
  ...
  if (submitted) {
    return (
      ...
      >
        ...
        >
          <div className="absolute inset-0 rounded-full" style={{ border: `1px solid rgba(${floor.accentRgb}, 0.3)` }} />
          <svg width="24" height="24" viewBox="0 0 28 28" fill="none" stroke={floor.accent} strokeWidth="1.5">
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
          className="font-cormorant mt-3 text-sm tracking-wider" style={{ color: `rgba(${floor.accentRgb}, 0.5)` }}>{t.successSub}</motion.p>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-10 flex gap-4">
          ...
          <button onClick={() => { setSubmitted(false); setFormData({ date: "", time: "", name: "", phone: "", email: "", notes: "" }); }}
            className="cta" style={{ borderColor: `rgba(${floor.accentRgb}, 0.4)`, color: floor.accent }}>{t.another}</button>
        </motion.div>
      </motion.div>
    );
  }
  ...
  /* ── Form ── */
  return (
    <div className="book-page" style={{ background: floor.bg, transition: "background 0.8s ease" }}>
      {/* Glow */}
      <div className="absolute inset-0 pointer-events-none transition-all duration-1000" style={{ background: floor.glow }} />

      {/* Ghost text */}
      <AnimatePresence mode="wait">
        <motion.div key={selectedFloor}
        ...
        >
          <span
            ...
            style={{ fontSize: "clamp(7rem, 25vw, 18rem)", color: `rgba(${floor.accentRgb}, 0.03)` }}
          >{floor.ghostText}</span>
        </motion.div>
      </AnimatePresence>
      ...
      {/* Form content */}
      <div className="relative z-10 flex flex-col h-full mx-auto max-w-lg px-6 sm:px-8">

        {/* Header */}
        <motion.div initial="hidden" animate="show" className="pt-4 sm:pt-6 mb-auto">
          <Link href="/" className="inline-flex items-center gap-1.5 font-cormorant text-[0.6rem] tracking-[0.35em] uppercase text-white/20 transition hover:text-white/50">
            <svg width="12" height="12" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1"><path d="M8 3L4 7L8 11" /></svg>
            {t.back}
          </Link>
        </motion.div>
        ...
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          className="font-instrument text-5xl sm:text-6xl tracking-wide text-white text-center mb-6 sm:mb-8"
        >
          {t.title}
        </motion.h1>

        {/* Form */}
        <motion.form onSubmit={handleSubmit} className="flex flex-col gap-4 sm:gap-5 mb-auto" initial="hidden" animate="show">

          {/* Floor tabs */}
          <motion.div custom={0} variants={rise} className="flex gap-1">
            {(Object.keys(floorConfig) as FloorId[]).map((fId) => {
              ...
              return (
                <button key={fId} type="button" onClick={() => setSelectedFloor(fId)}
                  className="flex-1 py-2.5 text-center font-cormorant text-[0.65rem] sm:text-xs tracking-[0.15em] uppercase transition-all duration-500"
                  style={{
                    color: active ? f.accent : "rgba(255,255,255,0.2)",
                    borderBottom: `1px solid ${active ? `rgba(${f.accentRgb}, 0.5)` : "rgba(255,255,255,0.06)"}`,
          ...
          {/* Date + Time + Guests — single row */}
          <motion.div custom={1} variants={rise} className="grid grid-cols-3 gap-4">
            <div>
              <label className="book-label" style={{ color: `rgba(${floor.accentRgb}, 0.6)` }}>{t.date}</label>
              <input type="date" required value={formData.date}
                onChange={(e) => setFormData({ ...formData, date: e.target.value })}
                className="book-input" style={{ borderColor: `rgba(${floor.accentRgb}, 0.2)` }} />
            </div>
            <div>
              <label className="book-label" style={{ color: `rgba(${floor.accentRgb}, 0.6)` }}>{t.time}</label>
              <input type="time" required value={formData.time}
                onChange={(e) => setFormData({ ...formData, time: e.target.value })}
                className="book-input" style={{ borderColor: `rgba(${floor.accentRgb}, 0.2)` }} />
            </div>
            <div>
              <label className="book-label" style={{ color: `rgba(${floor.accentRgb}, 0.6)` }}>{t.guests}</label>
              <div className="flex items-center gap-3 mt-1">
                <button type="button" onClick={() => setGuestCount(Math.max(1, guestCount - 1))}
                  className="font-cormorant text-lg transition" style={{ color: `rgba(${floor.accentRgb}, 0.4)` }}>−</button>
                <span className="font-instrument text-2xl" style={{ color: floor.accent, transition: "color 0.5s" }}>{guestCount}</span>
          ...
          <motion.div custom={2} variants={rise}>
            <label className="book-label" style={{ color: `rgba(${floor.accentRgb}, 0.6)` }}>{t.name}</label>
            <input type="text" autoComplete="name" required value={formData.name}
              onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              className="book-input" style={{ borderColor: `rgba(${floor.accentRgb}, 0.2)` }} />
          </motion.div>

          {/* Phone + Email — single row */}
          <motion.div custom={3} variants={rise} className="grid grid-cols-2 gap-4">
            <div>
              <label className="book-label" style={{ color: `rgba(${floor.accentRgb}, 0.6)` }}>{t.phone}</label>
              <input type="tel" inputMode="tel" autoComplete="tel" required value={formData.phone}
                onChange={(e) => setFormData({ ...formData, phone: e.target.value })}
                className="book-input" style={{ borderColor: `rgba(${floor.accentRgb}, 0.2)` }} />
            </div>
            <div>
              <label className="book-label" style={{ color: `rgba(${floor.accentRgb}, 0.6)` }}>{t.email}</label>
          ...
          <motion.div custom={4} variants={rise}>
            <label className="book-label" style={{ color: `rgba(${floor.accentRgb}, 0.6)` }}>{t.notes}</label>
          ...
          <AnimatePresence>
            {error && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                className="font-cormorant text-sm text-center" style={{ color: "#c84466" }}>{error}</motion.div>
            )}
          </AnimatePresence>
          ...
          <motion.div custom={5} variants={rise}>
            <button type="submit" disabled={submitting}
              className="group relative w-full overflow-hidden py-4 font-cormorant text-sm sm:text-base tracking-[0.3em] uppercase transition-all duration-500"
              style={{
